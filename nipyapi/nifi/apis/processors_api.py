...
class ProcessorsApi(object):
    ...
    def clear_bulletins5(self, body, id, **kwargs):
        """
        Clears bulletins for a processor.
        This method makes a synchronous HTTP request and returns the response data directly.
        
        For full HTTP response details (status code, headers, etc.), use the corresponding 
        ``clear_bulletins5_with_http_info()`` method instead.
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.clear_bulletins5_with_http_info(body, id, **kwargs)
        else:
            (data) = self.clear_bulletins5_with_http_info(body, id, **kwargs)
            return data

    def clear_bulletins5_with_http_info(self, body, id, **kwargs):
        """
        Clears bulletins for a processor.
        This method makes a synchronous HTTP request and returns detailed response information.
        
        Returns the response data along with HTTP status code, headers, and other metadata.
        For just the response data, use the corresponding ``clear_bulletins5()`` method instead.
        ...
        for key, val in params['kwargs'].items():
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method clear_bulletins5" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'body' is set
        if ('body' not in params) or (params['body'] is None):
            raise ValueError("Missing the required parameter `body` when calling `clear_bulletins5`")
        # verify the required parameter 'id' is set
        if ('id' not in params) or (params['id'] is None):
            raise ValueError("Missing the required parameter `id` when calling `clear_bulletins5`")
        ...
        return self.api_client.call_api('/processors/{id}/bulletins/clear-requests', 'POST',
                                        path_params,
                                        query_params,
                                        header_params,
                                        body=body_params,
