"""
For interactions with the NiFi Registry Service.

This module provides functions for working with NiFi Registry, a standalone
registry server for versioned flows. For Git-based registries (GitHub, GitLab,
Bitbucket, Azure DevOps), see the corresponding functions in nipyapi.versioning.

Note:
    NiFi Registry is receiving minimal maintenance. For new deployments,
    consider using Git-based flow registries instead.
"""
...
def update_flow_ver(process_group, target_version=None):
    ...
    with nipyapi.utils.rest_exceptions():
        vci = nipyapi.versioning.get_version_info(process_group)
...
def deploy_flow_version(parent_id, location, bucket_id, flow_id, reg_client_id, version=None):
    ...
    target_reg_client = nipyapi.versioning.get_registry_client(reg_client_id, "id")
