...
class AccessApi(object):
    ...
    def create_access_token_using_form_login(self, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.create_access_token_using_form_login(callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.create_access_token_using_form_login_with_http_info(**kwargs)
        else:
            (data) = self.create_access_token_using_form_login_with_http_info(**kwargs)
            return data

    def create_access_token_using_form_login_with_http_info(self, **kwargs):
        """
        Creates a token for accessing the REST API via username/password
        The user credentials must be passed in standard HTTP Basic Auth format. That is: 'Authorization: Basic <credentials>', where <credentials> is the base64 encoded value of '<username>:<password>'. The token returned is formatted as a JSON Web Token (JWT). The token is base64 encoded and comprised of three parts. The header, the body, and the signature. The expiration of the token is a contained within the body. The token can be used in the Authorization header in the format 'Authorization: Bearer <token>'.
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.create_access_token_using_form_login_with_http_info(callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method create_access_token_using_form_login" % key
                )
            params[key] = val
        del params['kwargs']
